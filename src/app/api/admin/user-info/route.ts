...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from('users')
      .select('id, email, organization_id, organizations(name, website_url, file_search_store_name, show_branding, publishable_key, widget_line1, widget_line2, notification_email, widget_position)')
      .eq('id', user.id)
      .single()
